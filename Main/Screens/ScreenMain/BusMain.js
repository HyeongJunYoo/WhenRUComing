      ...
      position => {
        ...
        AsyncStorage.getItem('busstop', (err, result) => {
          ...
          for (var i = 1; i < busst[busstoplength].length - 1; i++) {
            sum += Bus1(
              busst[busstoplength][i].X,
              busst[busstoplength][i].Y,
              busst[busstoplength][i + 1].X,
              busst[busstoplength][i + 1].Y,
            );
            console.log(sum);
          }
          sum += Bus1(
            ...
            busst[busstoplength][0].X,
            busst[busstoplength][0].Y,
          );
          console.log(sum);
          ...
          const distance = sum;
          const nextStop = busstoplength;
          if (distance && nextStop) {
            console.log('서버로 좌표 전송 완료!');
            bus.doc(busNumber).update({distance: distance});
            bus.doc(busNumber).update({nextStop: nextStop});
          }
        });
