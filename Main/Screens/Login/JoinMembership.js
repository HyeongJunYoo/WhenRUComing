 ...
 function JoinMembership({navigation}) {
  ...
  const signUpSubmit = async () => { // 회원가입 함수
    const {userId, userPassword, confirmPassword} = form;
    const info = {userId, userPassword, confirmPassword};
    console.log(form);
    console.log(info);
    try {
  ...
  const handleSubmitPress = () => {
    ...
    if (!form.userId) {
      ...
      signUpSubmit(!form.userPassword == !form.confirmPassword);
      }
  };
